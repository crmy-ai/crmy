# Provider Certification Checklist for 0.9.4
...
1. Start from a migrated 0.9.4 database.
2. Configure `CRMY_PUBLIC_URL` to the externally reachable CRMy base URL.
3. Open **Settings -> Context Connectors -> OAuth** as an admin.
...
6. Process the message as a Source.
7. Verify Sources, Signals, Memory candidates, and Lineage point back to the email message and mailbox connection.
...
7. Verify it becomes a meeting artifact, Sources, Signals, and reviewable Memory candidates.
...
- Redirect URI mismatch returns a friendly setup error and points admins to Context Connectors.
...
- Source lineage remains traceable from provider object to Signals and Memory.
