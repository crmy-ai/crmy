...
- **Output**: `{ action_context: { contract_version, operating_mode, readiness, policy?, source_posture, allowed_actions, human_unblock?, proof, next_tools, context_packing, guidance, action_packet, briefing, checks, required_handoffs } }`
- **Stable contract**: `contract_version: "crmy.action_context.v1"` identifies the portable agent preflight contract returned identically over MCP, REST, CLI `--json`, and Workspace Agent internals. The frozen v1 fields are `operating_mode`, `readiness`, `policy`, `source_posture`, `allowed_actions`, `human_unblock`, `proof`, `next_tools`, and `context_packing`. Additional fields such as `briefing`, `checks`, and `action_packet` are richer compatibility/detail surfaces.
