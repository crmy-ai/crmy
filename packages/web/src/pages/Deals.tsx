...
const sortOptions: SortOption[] = [
  { key: 'name', label: 'Deal Name' }, { key: 'amount', label: 'Amount' },
        ...
        {isLoading ? (
          ...
          <div className="flex gap-3 md:gap-4 px-4 md:px-6 pb-4 overflow-x-auto min-h-full snap-x snap-mandatory md:snap-none no-scrollbar">
            {kanbanStages.map((stage) => {
              ...
              const total = stageDeals.reduce((sum, d) => sum + ((d.amount as number) || 0), 0);
              return (
                <div key={stage} className="flex-shrink-0 w-[280px] md:w-72 flex flex-col snap-center">
                  ...
                  <div className="flex-1 space-y-2">
                    {stageDeals.map((deal, i) => {
                      ...
                      const daysInStage = (deal.days_in_stage ?? deal.daysInStage ?? 0) as number;
                      return (
                        <motion.div key={deal.id as string} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                          ...
                          <div className="flex items-start justify-between">
                            <p className="text-sm font-display font-bold text-foreground">{deal.name as string}</p>
                            <button onClick={(e) => { e.stopPropagation(); openAIWithContext({ type: 'deal', id: deal.id as string, name: deal.name as string, detail: `$${(amount / 1000).toFixed(0)}K` }); navigate('/agent'); }}
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.map((d, i) => {
                        ...
                        return (
                          <tr key={d.id as string} onClick={() => openDrawer('deal', d.id as string)}
                            ...
                            <td className="px-2 py-3">
                              <button onClick={(e) => { e.stopPropagation(); openAIWithContext({ type: 'deal', id: d.id as string, name: d.name as string, detail: `$${(amount / 1000).toFixed(0)}K` }); navigate('/agent'); }}
