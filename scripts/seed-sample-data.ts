...
async function main() {
  ...
  await insertAssignment(db, tenantId, { title: 'Book Summit Consulting upsell discovery call',          assignment_type: 'call',      assigned_by: agentActor.id,   assigned_to: jordanActor.id, subject_type: 'account',     subject_id: summit.id,        priority: 'normal', status: 'completed',   context: 'Grace confirmed interest in governed context add-on. Call completed, demo scheduled.' });
