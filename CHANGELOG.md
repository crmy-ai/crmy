...
- **Governed Product Knowledge Retrieval — Phase 1** — `knowledge_retrieve` MCP tool + `KnowledgeRetrievalService` contract and `product_knowledge` toolset. Optional and non-blocking: returns a clear `not_configured` until a product-knowledge source is configured; never creates Memory or writes to systems of record. Retrieval over sources lands in Phase 2.
